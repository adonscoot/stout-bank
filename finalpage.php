...
<body>
    ...
    <main>
        <div class="create-account-container">
            ...
            <?php
            $sql = "SELECT notification_id, date_earned FROM user_notifications WHERE routing_number = ?";
            $stmt = $conn->prepare($sql);
            if (!$stmt) {
                die("Prepare failed: " . $conn->error);
            }
            ...
            while ($row = $result->fetch_assoc()) {
                $notificationCount++;
                if ($row['notification_id'] == 1) {
                    $dateEarned1 = new DateTime($row['date_earned']);
                } elseif ($row['notification_id'] == 9) {
                    $dateEarned9 = new DateTime($row['date_earned']);
                }
            }

            if ($dateEarned1 && $dateEarned9) {
                $interval = $dateEarned1->diff($dateEarned9);
                echo "It took " . $interval->h . " hours, and " . $interval->i . " minutes to complete the challenge.";
            } else {
                echo "The required notifications are not completed yet.";
            }

            echo "<br>You have completed $notificationCount out of 9 achievements.<br>";

            echo "<a href='/StoutBank/logout.php' class='button'>logout</a>";
            ?>
        </div>
    </main>
